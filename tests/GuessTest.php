...
use \Nuffy\wordle\models\{GuessResult, Word, GuessedLetter};
...
class GuessTest extends TestCase
{
    ...
    function testCanLocateRightLetters(){
        ...
        $this->assertEquals(GuessedLetter::CORRECT_PLACEMENT, $guess_result->getLetter(0)->status);
        $this->assertEquals(GuessedLetter::WRONG_LETTER, $guess_result->getLetter(1)->status);
        $this->assertEquals(GuessedLetter::WRONG_LETTER, $guess_result->getLetter(2)->status);
        $this->assertEquals(GuessedLetter::CORRECT_LETTER, $guess_result->getLetter(3)->status);
    }
    ...
    function testCanHandleMultiplePresentLetters(){
        ...
        $this->assertEquals(GuessedLetter::CORRECT_PLACEMENT, $guess_result->getLetter(0)->status);
        $this->assertEquals(GuessedLetter::CORRECT_LETTER, $guess_result->getLetter(4)->status);
        $this->assertEquals(GuessedLetter::WRONG_LETTER, $guess_result->getLetter(3)->status);

        $guess_result = GuessController::makeGuess($answer, new Word("keeps"));
        $this->assertEquals(GuessedLetter::CORRECT_LETTER, $guess_result->getLetter(1)->status);
        $this->assertEquals(GuessedLetter::WRONG_LETTER, $guess_result->getLetter(2)->status);

        $guess_result = GuessController::makeGuess($answer, new Word("sweet"));
        $this->assertEquals(GuessedLetter::WRONG_LETTER, $guess_result->getLetter(2)->status);
        $this->assertEquals(GuessedLetter::CORRECT_PLACEMENT, $guess_result->getLetter(3)->status);
    }
}